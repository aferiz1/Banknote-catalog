...
const denominations = {
  findDenominationsForCurrency(currencyId: number) {
    return (`SELECT
  amount,
  description
FROM
  denominations 
WHERE valid = 1 AND currencyId=${currencyId};`);
  },

  editDenomination(obj: any) {
    return (`UPDATE
    denominations AS den,
    currencies AS crr
  SET
    den.currencyId = crr.id,
    den.amount = ${obj.amount},
    den.description = '${obj.description}',
    den.dateModified = NOW(),
    den.userModified = 'SYSTEM'
  WHERE
	den.id=${obj.id} AND crr.name='${obj.currencyName}';`)
  },

  addDenomination(obj: any) {
    return (`INSERT INTO denominations(
      amount,
      description,
      userCreated,
      dateCreated,
...
const queries = Object.assign({}, { users, currencies, denominations, banknotes, countries });
